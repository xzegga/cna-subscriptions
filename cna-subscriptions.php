...
define('CNA_SUBSCRIPTIONS_VERSION', '1.0.1');
...
spl_autoload_register(function ($class) {
    ...
    // Mapeos donde el nombre de clase no coincide con el nombre del archivo
    $class_file_map = array(
        'CNA_REST_Controller' => 'includes/API/class-cna-rest.php',
        'CNA_Pagadito_Webhook' => 'includes/API/class-cna-pagadito-webhook.php',
    );

    if (isset($class_file_map[$class])) {
        $file_path = CNA_SUBSCRIPTIONS_PLUGIN_DIR . $class_file_map[$class];
...
/**
 * Ejecuta migraciones lo antes posible en cada request.
 */
function cna_subscriptions_run_db_migrations()
{
    if (class_exists('CNA_Migrator')) {
        CNA_Migrator::migrate();
    }
}
add_action('plugins_loaded', 'cna_subscriptions_run_db_migrations', 1);

/**
 * Aviso en admin si la migración de pagadito_ern no se aplicó (p. ej. permisos ALTER denegados).
 */
function cna_subscriptions_admin_db_notice()
{
    if (!is_admin() || !current_user_can('manage_options')) {
        return;
    }

    if (!class_exists('CNA_Migrator') || CNA_Migrator::subscriptions_table_has_pagadito_ern()) {
        return;
    }
    ...
    echo '<div class="notice notice-error"><p><strong>CNA Subscriptions:</strong> ';
    echo esc_html(sprintf(
        'Falta la columna pagadito_ern en %s. Revisa permisos ALTER en la base de datos o ejecuta manualmente: ALTER TABLE `%s` ADD COLUMN pagadito_ern varchar(50) DEFAULT NULL;',
        $table,
        $table
    ));
    echo '</p></div>';
}
add_action('admin_notices', 'cna_subscriptions_admin_db_notice');
...
function cna_subscriptions_load_classes()
{
    $classes = array(
        ...
        'CNA_Security' => 'includes/Model/class-cna-security.php',
        'CNA_Pages' => 'includes/Core/class-cna-pages.php',
